...
impl FileSystem {
    ...
    pub fn print_block(&mut self, quantidade: &String, vec_index: &usize) -> (){
        let blocks_index = self.fs[*vec_index].index;
        let quantidade_int = quantidade.parse::<i32>().unwrap();
        for i in blocks_index..blocks_index+quantidade_int {
            let tmp_i = i as usize;
            self.blocks[tmp_i] = Block::Occupied;
        }
    }

    pub fn print_remove_block(&mut self, nome: &String) -> (){
        ...
        for i in block_index..block_index+block_size{
            let tmp_i = i as usize;
            self.blocks[tmp_i] = Block::Free;
        }
    }

    pub fn get_file_index(&self, filename: &String) -> (bool, usize) {
        for i in 0..self.fs.len() {
    ...
    pub fn storage_available(&self, file_size: i32) -> (bool, usize) {        
        for i in 0..self.fs.len() {
    ...
    pub fn create_file(&mut self, pid: &u16, filename: &String, filesize: &i32, vec_index: &usize) -> () {
        let file = File{
            free: true,
            file_name: (*filename.clone()).to_string(),
            file_owner: *pid,
    ...
    pub fn delete_file(&mut self, pid: &u16, process_priority: &usize, filename: &String) -> bool {
